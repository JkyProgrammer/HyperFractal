...
equation* extract_equation (string sequ) {
    ...
    else if (compos[0].find('i') != -1) e->a = value (cpp_complex_512{0,stod(compos[0].substr(0,compos[0].length()-1))});
    ...
    else if (compos[2].find('i') != -1) e->b = value (cpp_complex_512{0,stod(compos[2].substr(0,compos[2].length()-1))});
